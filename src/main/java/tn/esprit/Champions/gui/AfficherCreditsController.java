...
public class AfficherCreditsController implements Initializable {
    ...
    @FXML
    private TableColumn<credit, String> colProjet;
    @FXML
    private TableColumn<credit, Double> colMontant;
    @FXML
    private TableColumn<credit, String> colDevise;
    @FXML
    private TableColumn<credit, Double> colTaux;
    @FXML
    private TableColumn<credit, Integer> colDuree;
    @FXML
    private TableColumn<credit, CreditStatus> colStatus;
    @FXML
    ...
    // ✅ Cache pour stocker les noms des projets et permettre une recherche
    // instantanée
    ...
    private void configurerColonnes() {
            ...
            protected void updateItem(CreditStatus item, boolean empty) {
                ...
                if (empty || item == null)
                    ...
                    if (item == CreditStatus.OPEN)
                        badge.setStyle(styleBase
                                + "-fx-background-color: rgba(243, 156, 18, 0.2); -fx-text-fill: #f39c12; -fx-border-color: rgba(243, 156, 18, 0.3); -fx-border-radius: 20;");
                    else if (item == CreditStatus.APPROVED)
                        badge.setStyle(styleBase
                                + "-fx-background-color: rgba(16, 185, 129, 0.2); -fx-text-fill: #10b981; -fx-border-color: rgba(16, 185, 129, 0.3); -fx-border-radius: 20;");
                    else if (item == CreditStatus.REJECTED)
                        badge.setStyle(styleBase
                                + "-fx-background-color: rgba(231, 76, 60, 0.2); -fx-text-fill: #e74c3c; -fx-border-color: rgba(231, 76, 60, 0.3); -fx-border-radius: 20;");
                    else
                        badge.setStyle(styleBase
                                + "-fx-background-color: rgba(148, 163, 184, 0.2); -fx-text-fill: #94a3b8; -fx-border-color: rgba(148, 163, 184, 0.3); -fx-border-radius: 20;");
        ...
        colActions.setCellFactory(param -> new TableCell<>() {
            ...
            {
                btnOffers.setStyle(
                        "-fx-background-color: rgba(56, 189, 248, 0.1); -fx-border-color: #38bdf8; -fx-border-radius: 12; -fx-background-radius: 12; -fx-text-fill: #38bdf8; -fx-font-weight: 900; -fx-cursor: hand; -fx-padding: 5 15;");
                btnOffers.setOnAction(event -> ouvrirNegociationEmprunteur(getTableView().getItems().get(getIndex())));

                // Effet Hover simple en code
                btnOffers.setOnMouseEntered(e -> btnOffers.setStyle(
                        "-fx-background-color: #38bdf8; -fx-text-fill: #020617; -fx-background-radius: 12; -fx-font-weight: 900;"));
                btnOffers.setOnMouseExited(e -> btnOffers.setStyle(
                        "-fx-background-color: rgba(56, 189, 248, 0.1); -fx-border-color: #38bdf8; -fx-border-radius: 12; -fx-background-radius: 12; -fx-text-fill: #38bdf8; -fx-font-weight: 900;"));
            }
    ...
    private void configurerRechercheDynamique() {
        ...
        for (CreditStatus s : CreditStatus.values())
    ...
    private void chargerDonnees() {
        try {
            ...
            for (projet p : ps.SelectAll()) {
                projetCache.put(p.getId_projet(), p.getTitle());
            }
    ...
    private void updateStatLabel(int count) {
        if (statLabel != null) {
            ...
            statLabel.setStyle(
                    "-fx-background-color: rgba(56, 189, 248, 0.1); -fx-text-fill: #38bdf8; -fx-padding: 8 20; -fx-background-radius: 20; -fx-font-weight: bold;");
        }
    }
    ...
    private void ouvrirAjout() {
        try {
            ...
            afficherErreur("Erreur FXML : " + e.getMessage());
        }
    }
