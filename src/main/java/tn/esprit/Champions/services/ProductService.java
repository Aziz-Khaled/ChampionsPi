...
public class ProductService implements CRUD<Product> {
    ...
    public void insertOne(Product product) throws SQLException {
        ...
        try (PreparedStatement pstmt = cnx.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            ...
            pstmt.setString(9, product.getCategory().name().toLowerCase());
            pstmt.setString(10, product.getStatus().name().toLowerCase());
            pstmt.setInt(11, userId); // ✅ was: product.getUserId()
    ...
    public void updateOne(Product product) throws SQLException {
        ...
        int userId = UserSession.getLoggedInUser().getId_user(); // ✅ Add this
    ...
    public void decrementStock(Integer productId, int quantity) throws SQLException {
        if (cnx == null)
            throw new SQLException("Connexion à la base de données non établie.");
        ...
        try (Statement stmt = cnx.createStatement();
            ...
            while (rs.next()) {
                ...
                if (categoryStr != null && !categoryStr.isEmpty()) {
                    try {
                        String cleanCat = categoryStr.toUpperCase();
                        if (cleanCat.equals("SUBSCRIPTION")) cleanCat = "SUBSCRIPTIONS";
                        category = ProductCategory.valueOf(cleanCat);
