...
import tn.esprit.Champions.models.TradeType; // Assure-toi d'importer ton enum TradeType
...
public class wallet_currencyService implements CRUD<wallet_currency> {
    ...
    /**
     * NOUVELLE MÉTHODE : Mise à jour automatique du solde après un Trade
     * Utilisée par le Bot pour impacter le portefeuille immédiatement.
     */
    public void updateBalanceAfterTrade(int walletId, int currencyId, double amount, TradeType type) throws SQLException {
        // 1. Récupérer le solde actuel
        double currentBalance = getBalance(walletId, currencyId);
        double newBalance;

        // 2. Calculer le nouveau solde selon le type d'opération
        if (type == TradeType.BUY) {
            newBalance = currentBalance - amount;
            if (newBalance < 0) throw new SQLException("Solde insuffisant pour effectuer l'achat.");
        } else {
            newBalance = currentBalance + amount;
        }
        ...
        try (PreparedStatement pst = cnx.prepareStatement(query)) {
            ...
            pst.executeUpdate();
            System.out.println("💰 Portefeuille mis à jour : " + currentBalance + " -> " + newBalance);
        }

        updateWalletModificationDate(walletId);
    }
    ...
    public void updateOne(wallet_currency wc) throws SQLException {
        ...
        try (PreparedStatement pst = cnx.prepareStatement(query)) {
            ...
            pst.executeUpdate();
            System.out.println("✅ [DB] Solde forcé : " + wc.getSolde());
        }
        ...
        try (PreparedStatement pst = cnx.prepareStatement(selectQuery)) {
            ...
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    ...
                    if (solde > 0) throw new SQLException("Impossible de supprimer : solde non nul !");
                }
            }
        }
